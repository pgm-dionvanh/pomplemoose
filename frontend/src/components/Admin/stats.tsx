...
import { CATEGORYS, CategorysQueryResponse } from "../../graphql/categorys";
import {  PRODUCTS, ProductsQueryResponse } from "../../graphql/products";
import { USERS, UsersQueryResponse } from "../../graphql/users";

export default function AdminStats() {
const products = useQuery<ProductsQueryResponse>(PRODUCTS);
const categorys = useQuery<CategorysQueryResponse>(CATEGORYS);
const users = useQuery<UsersQueryResponse>(USERS);
return (
<>
...
<div className="m-4 grid grid-cols-1 md:grid-cols-4 gap-4">
    <div
        className="flex flex-col justify-center items-center col-span-1 bg-white p-6 shadow-lg hover:shadow-none cursor-pointer transition-all duration-300 ease-in border-b-4 border-opacity-0 hover:border-opacity-100 border-indigo-200">
    ...
    <div
        ...
        <div className="text-2xl font-semibold">
            { categorys.data?.categories.length }
        </div>
